...
| `run_retriever_v1.py` | 命令行方式验证 `RetrieverV1` 的检索效果。 | 简历/JD 文本与 `top-k` 参数 | 控制台输出检索结果 JSON（调试用） |
| `evaluate_retriever_v1.py` | 离线评测 `RetrieverV1` 指标（相关性 + 质量维度）。 | 人工标注用例 JSONL（query + 相关题ID/相关度 + 公司岗位 + 简历关键词） | 评测报告 JSON（summary + per_case） |
...
| `run_retriever_v2.py` | 运行 `RetrieverV2`（Qdrant 向量召回 + v1 词法融合 + 质量重排）。 | 简历/JD 文本 + Qdrant 参数 + embedding 参数 | 控制台输出检索结果 JSON |
...
| 查看检索效果是否符合预期 | `run_retriever_v1.py` |
| 量化检索质量并对比版本效果 | `evaluate_retriever_v1.py` |
...
11. `embedding_provider=hash` 可离线跑通；`embedding_provider=openai_compatible` 可接入在线 embedding 接口提升语义效果。

## 检索评测样例（evaluate_retriever_v1）
...
```powershell
C:\Users\32014\miniconda3\python.exe backend\scripts\evaluate_retriever_v1.py `
  --cases data\nowcoder\eval\retriever_cases.jsonl `
  --dataset data\nowcoder\pipeline_runs_llm\canonical\long_content_retrieval_questions.jsonl `
  --retriever-backend v1 `
  --top-k 8 `
  --freshness-half-life-days 180 `
  --output data\nowcoder\pipeline_runs_llm\eval\retriever_v1_eval_report.json
```
