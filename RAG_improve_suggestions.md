...
| 已完成 | 30 条关键词银标评测 | 生成并修正 `data/nowcoder/eval/retriever_cases.silver_keyword_30.jsonl`，基于题库真实 `question_id` 构造 30 条 Java 后端主题银标 case。修正点：银标生成改为归一化匹配，避免 `SpringBoot` / `Spring Boot`、符号/空格差异造成假阴性。当前主题强约束版本指标：Hit@8=1.0、Precision@8=0.3625、Recall@8=0.370833、MRR@8=0.808968、MAP@8=0.280617、NDCG@8=0.46954、Diversity@8=1.0、Quality=0.677731。最差主题集中在 JVM GC、GC 算法、Redis 分布式锁、消息队列不丢失、并发锁、数据库设计等，说明系统已基本能碰到相关题，但 Top8 内相关题数量和排序仍不足。报告写入 `data/nowcoder/eval/retriever_v2_silver_keyword_30_report.json`。 |
| 已完成 | 主题强约束 rerank 第一版 | 在 `backend/interview/retriever_v2.py` 中新增 `topic_coverage`：从 query/JD/profile 抽取核心技术主题词，在粗排和终排中提升精确主题覆盖更高的候选题，并将命中词写入 `matched_keywords`、将覆盖分写入 `score_breakdown.topic_coverage`。初版 29 条银标对照中，Hit@8 从 0.931034 提升到 0.965517，Precision@8 从 0.267241 提升到 0.344828，Recall@8 从 0.272167 提升到 0.349754，NDCG@8 从 0.34814 提升到 0.428193；随后发现 SpringBoot 标签归一化问题并修正为 30 条银标，当前指标见上一行。`python -m py_compile interview/retriever_v2.py` 与 `pytest tests/test_auth_and_interview_api.py -q` 通过。 |
